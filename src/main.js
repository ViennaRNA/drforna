...
function start() {
    ...
    document.querySelectorAll('.fileinput').forEach((item)=>{
        ...
        item.addEventListener('change', (event)=>{
            ...
            d3.text(""+event.target.files[0].name)
            .then(d=>{
            let a=d3.csvParse(d.replace(/ +/g, ","))
...
function ShowData(data){
 ...
 let viscontainer=d3.select('#visContainer')            
    ...
    var nestedData =  Array.from(d3.group(filteredData, d => d.time))//.filter(d=>{d.conc > occupancyTreshold}))
        //console.log(nestedData) // when I have the time I can use get("time") to get the list of structures for that time
    ...
    play.on("click", function(event, d){
        console.log(event, d, "button clicked")
        PLOT(0.4)
            let delay = 0;
            nestedData.forEach(element=>{
                setTimeout(() => {
                    ...
                    svg.selectAll("line").remove()
                    svg.append("line")
                        .attr("x1", scale.invert(element[0]))  //<<== change your code here
                        .attr("y1", 0)
                        .attr("x2", scale.invert(element[0]))  //<<== and here
                        .attr("y2", 120)
                        .style("stroke-width", 2)
                        .style("stroke", "red")
                        .style("fill", "none");
   ...
   svg.on("mouseover", function(event, d) {
    ...
    //console.log(realtime)
       ...
       svg.selectAll("line").remove()
       svg.append("line")
            .attr("x1", d3.pointer(event)[0])  //<<== change your code here
            .attr("y1", 0)
            .attr("x2", d3.pointer(event)[0])  //<<== and here
            .attr("y2", 120)
            .style("stroke-width", 2)
            .style("stroke", "red")
            .style("fill", "none");}
